...
for epoch in range(args.epochs):
    for (images, _) in tqdm(train_loader):
        ...
        enc_optim.step()
        dec_optim.step()
        # print("step = %d, total_loss = %f, recon_loss = %f" %(step, total_loss.data.item(), recon_loss.data.item()))
        step += 1
        ...
        logging_info = {
                        'AE loss': total_loss.item(),
                        'input_reconst loss': recon_loss.item()
                    }
        ...
        # images
        if step % 1000 == 0:
            tag = "MNIST_WAE_MMD_OTHERS_DcEnCoder_%d" %step
            print(tag)
            writer.add_images(tag, x_recon.view(batch_size, 1, 28, 28).data, step)
